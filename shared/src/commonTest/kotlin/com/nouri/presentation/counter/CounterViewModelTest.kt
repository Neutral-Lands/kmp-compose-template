...
class CounterViewModelTest {
    ...
    @Test
    fun `given count is zero when increment then count is one`() =
        runTest {
            val viewModel = CounterViewModel()
    ...
    @Test
    fun `given count is zero when decrement then count stays zero`() =
        runTest {
            val viewModel = CounterViewModel()
    ...
    @Test
    fun `given count is at limit when increment then emits LimitReached effect`() =
        runTest {
            val viewModel = CounterViewModel()
    ...
    @Test
    fun `given count is at limit when increment then count stays at limit`() =
        runTest {
            val viewModel = CounterViewModel()
            repeat(10) { viewModel.handleIntent(CounterIntent.Increment) }
            viewModel.handleIntent(CounterIntent.Increment)
