...
class BaseViewModelErrorTest {
    @Test
    fun `when handleError called then error flow emits UnknownError`() =
        runTest {
    ...
    @Test
    fun `when handleError called then emitted error wraps original throwable`() =
        runTest {
            val viewModel = TestViewModel()
    ...
    @Test
    fun `multiple handleError calls emit multiple errors`() =
        runTest {
            val viewModel = TestViewModel()
