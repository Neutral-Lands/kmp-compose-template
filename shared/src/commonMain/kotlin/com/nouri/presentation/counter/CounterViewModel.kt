...
class CounterViewModel : BaseViewModel<CounterState, CounterIntent, CounterAction, CounterEffect>(
    ...
    override suspend fun processAction(action: CounterAction) {
        when (action) {
            CounterAction.Increment ->
                if (currentState.count >= COUNT_LIMIT) emitEffect(CounterEffect.LimitReached)
